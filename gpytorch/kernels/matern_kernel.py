    ...
    def forward(self, x1, x2, diag=False, **params):
        if (
            ...
            if self.active_dims is not None:
                # The active dims have their distance scaled, the 
                # non-active have their locations projected to zero
                # additive? 
                x1_ = compute_active_distance(x1, mean, self.lengthscale, self.active_dims)
                x2_ = compute_active_distance(x2, mean, self.lengthscale, self.active_dims)
            else:
                x1_ = (x1 - mean).div(self.lengthscale)
                x2_ = (x2 - mean).div(self.lengthscale)
            #Man vill få NER distansen mellan element, så att kovariansen mellan dem går upp.
            #Därför kommer man att explora mindre som resultat
        ...
        res = MaternCovariance.apply(
            x1, x2, self.lengthscale, self.nu, lambda x1, x2: self.covar_dist(x1, x2, **params)
        )
        return res


def compute_active_distance(X, mean, lengthscale, active_dims):
    act_dims = active_dims.unsqueeze(-2).to(lengthscale)
    # This makes the lengthscales infinitely long for the non-active dimensions
    # Which means that the distance between points will become just about zero along those dims
    return (X - mean).div(lengthscale / (act_dims + EPS))